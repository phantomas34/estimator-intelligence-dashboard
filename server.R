...
function(input, output, session) {
  ...
  output$date_verification_table <- renderDataTable({
    ...
    d %>% arrange(report_date) %>%
      mutate(`Raw Format (MoYear)` = format(report_date, "%y-%m"),
             `Parsed Date`         = format(report_date, "%B %Y"),
             `Full Date Stored`    = as.character(report_date)) %>%
      select(Estimator = primary_name, `Raw Format (MoYear)`, `Parsed Date`, `Full Date Stored`) %>%
      distinct() %>% head(50)
  ...
  its_series <- reactive({
    ...
    req(nrow(df) > 0, input$its_estimator, metric)
  ...
  observeEvent(input$its_run, {
    ...
    if (autodetect) {
      tryCatch({
        ...
        if (!any(is.na(bp_idx)) && length(bp_idx) > 0) {
          ...
          fstat_at_bp <- sapply(bp_idx, function(i) {
            pos <- which(fs_times == i)
            if (length(pos) > 0) round(fs_vals[pos], 2) else NA_real_
          })
          
          bp_table <- tibble(
            Date            = bp_dates,
            `Time Index`    = bp_idx,
            `F-Statistic`   = fstat_at_bp,
            `Months Before` = bp_idx,
            `Months After`  = nrow(series) - bp_idx
          )
          # ── END BUG FIX 3 ──────────────────────────────────────────────
        }
    ...
    if (!is.null(manual_dt) && !is.na(manual_dt) &&
        manual_dt >= min(series$date_group) && manual_dt <= max(series$date_group)) {
    ...
    cf_df <- data.frame(
      Term        = recode(rownames(cf),
                           ...
                           `t`           = "Pre-intervention trend",
                           `D`           = "Level change",
                           `t_post`      = "Slope change"),
      Estimate    = round(cf[, "Estimate"],   2),
      `Std Error` = round(cf[, "Std. Error"], 2),
      `t value`   = round(cf[, "t value"],    3),
      `p value`   = ifelse(cf[, "Pr(>|t|)"] < 0.001,
                           "< 0.001",
                           as.character(round(cf[, "Pr(>|t|)"], 3))),
      Significant = ifelse(cf[, "Pr(>|t|)"] < 0.05, "Yes *", "No"),
    ...
    b2 <- coef(fit)["D"]; b3 <- coef(fit)["t_post"]
    p2 <- cf[3, 4];       p3 <- cf[4, 4]
  ...
  output$its_summary_ui <- renderUI({
    ...
    tagList(
      tags$table(style = "width:100%; border-collapse:collapse; font-size:13px;",
                 tags$tr(
                   tags$td(style = "padding:6px 10px; font-weight:600; color:#2c3e50; width:40%;", "Estimator"),
                   tags$td(style = "padding:6px 10px; color:#5A6A72;", res$estimator)),
                 tags$tr(style = "background:#f4f6f9;",
                         tags$td(style = "padding:6px 10px; font-weight:600; color:#2c3e50;", "Outcome Metric"),
                         tags$td(style = "padding:6px 10px; color:#5A6A72;", metric_label[res$metric])),
                 tags$tr(
                   tags$td(style = "padding:6px 10px; font-weight:600; color:#2c3e50;", "Intervention Date"),
                   tags$td(style = "padding:6px 10px; color:#5A6A72;",
                           format(res$active_date, "%B %Y"),
                           if (!is.null(input$its_manual_date) && !is.na(input$its_manual_date) &&
                 ...
                 tags$tr(style = "background:#f4f6f9;",
                         tags$td(style = "padding:6px 10px; font-weight:600; color:#2c3e50;", "Model R\u00b2"),
  ...
  output$its_chart <- renderPlotly({
    ...
    fmt <- if (metric %in% c("amt_booked","amt_bid")) function(x) dollar(round(x)) else
      if (metric == "win_rate")                  function(x) percent(x, 0.1)  else
    ...
    p <- ggplot(series, aes(x = date_group)) +
      geom_point(aes(y = value,
                     text = paste0("<b>", format(date_group, "%b %Y"), "</b><br>Actual: ", fmt(value))),
      ...
      annotate("text", x = res$active_date, y = max(series$value, na.rm = TRUE) * 0.95,
               label = paste("Intervention\n", format(res$active_date, "%b %Y")),
               hjust = -0.08, size = 3, color = "#e67e22") +
      scale_y_continuous(labels = if (metric %in% c("amt_booked","amt_bid")) dollar_format(accuracy = 1) else
        if (metric == "win_rate") percent_format(accuracy = 1) else
          comma_format(accuracy = 1)) +
      labs(x = NULL, y = y_label,
           title   = paste0(res$estimator, " — ITS: ", y_label),
           caption = "Blue = pre-int. trend | Green = post-int. trend | Red dotted = counterfactual") +
      theme_minimal() +
  ...
  output$bid_funnel_chart <- renderPlotly({
    ...
    funnel <- d %>%
      group_by(primary_name) %>%
      summarise(Bid = sum(amt_bid, na.rm = TRUE), Booked = sum(amt_booked, na.rm = TRUE), .groups = "drop") %>%
      filter(Bid > 0) %>%
      mutate(Win_Rate = Booked / Bid, primary_name = fct_reorder(primary_name, Bid)) %>%
    ...
    p <- ggplot(funnel, aes(x = primary_name, y = Amount, fill = Type,
                            text = paste0("<b>", primary_name, "</b><br>", Type, ": ", dollar(Amount, accuracy = 1),
      ...
      scale_y_continuous(labels = dollar_format(accuracy = 1), expand = expansion(mult = c(0, 0.25))) +
      coord_flip() + labs(x = "", y = "Amount ($)", fill = "") +
  ...
  output$seasonality_heatmap <- renderPlotly({
    ...
    metric_sym   <- sym(hm_metric)
    metric_label <- if (hm_metric == "amt_booked") "Booked ($)" else "Bid ($)"
    
    heat <- df %>%
      mutate(Month = month(report_date, label = TRUE, abbr = TRUE),
             Year  = as.integer(year(report_date))) %>%
      group_by(Year, Month) %>%
      summarise(Value = sum(!!metric_sym, na.rm = TRUE), .groups = "drop") %>%
      complete(Year, Month, fill = list(Value = 0)) %>%
      mutate(Month = factor(Month, levels = rev(month.abb)))
    
    p <- ggplot(heat, aes(x = factor(Year), y = Month, fill = Value,
                          text = paste0("<b>", Month, " ", Year, "</b><br>", metric_label, ": ", dollar(Value, accuracy = 1)))) +
      geom_tile(color = "white", linewidth = 0.4) +
      scale_fill_gradient(low = "#eaf4fb",
                          high = if (hm_metric == "amt_booked") "#1a5276" else "#7f8c8d",
  ...
  output$scatter_matrix <- renderPlotly({
    ...
    ds_df <- dat %>%
      group_by(primary_name) %>%
      summarise(Total_Bid    = sum(amt_bid,    na.rm = TRUE),
                Total_Booked = sum(amt_booked, na.rm = TRUE),
                Jobs_Bid     = sum(qty_bid,    na.rm = TRUE),
                .groups = "drop") %>%
      mutate(Avg_Job_Size = Total_Bid / Jobs_Bid, Win_Rate_SM = Total_Booked / Total_Bid)
    ...
    ds_df <- ds_df %>%
      filter(Total_Bid > 0, !is.na(Win_Rate), !is.infinite(Win_Rate),
             !is.na(Avg_Job_Size), !is.infinite(Avg_Job_Size))
    validate(need(nrow(ds_df) >= 3, "Need at least 3 active estimators to cluster."))
    
    set.seed(42)
    if (any(sapply(ds_df[, c("Total_Bid", "Win_Rate", "Avg_Job_Size")], function(x) sd(x, na.rm = TRUE) == 0))) {
    ...
    p <- ggplot(ds_df, aes(x = Total_Bid, y = Win_Rate, color = Archetype, size = Total_Booked,
      ...
      scale_color_manual(values = c("High-Efficiency" = "#27ae60", "Selective" = "#f39c12", "High-Volume" = "#e74c3c")) +
  ...
  output$annual_bar_chart <- renderPlotly({
    ...
    x_levels <- d %>% arrange(report_date) %>% pull(x_group) %>% unique()
    d <- d %>% mutate(x_group = factor(x_group, levels = x_levels))
    
    if (by_person) {
      annual <- d %>% group_by(x_group, primary_name) %>% summarise(Booked = sum(amt_booked, na.rm = TRUE), .groups = "drop")
      p <- ggplot(annual, aes(x_group, Booked, fill = primary_name,
                              text = paste0("<b>", x_group, "</b><br>", primary_name, "<br>", dollar(round(Booked), accuracy = 1)))) +
      ...
      p <- ggplot(annual, aes(x_group, Value, fill = Type,
                              text = paste0("<b>", x_group, "</b><br>", Type, ": ", dollar(round(Value), accuracy = 1)))) +
        geom_col(position = "dodge") + scale_fill_manual(values = c("Bid" = "#95a5a6", "Booked" = "#27ae60"))
    }
    ggplotly(p + scale_y_continuous(labels = dollar_format(accuracy = 1)) +
  ...
  output$yoy_chart <- renderPlotly({
    ...
    bar_pos <- input$yoy_position
    
    if (input$view_mode == "Individual") {
      validate(need(length(unique(d$primary_name)) <= 10, "Select 10 or fewer estimators for Individual YoY."))
      ag <- d %>%
        group_by(year_num, primary_name) %>%
        summarise(V = sum(!!msym, na.rm = TRUE), .groups = "drop") %>%
        arrange(year_num) %>%
        group_by(primary_name) %>%
        mutate(G = (V - lag(V)) / lag(V)) %>%
        filter(!is.na(G))
      p <- ggplot(ag, aes(factor(year_num), G, fill = primary_name,
                          text = paste0(year_num, " — ", primary_name, "<br>Growth: ", percent(G, 0.1)))) +
        geom_col(position = bar_pos) + scale_fill_brewer(palette = "Paired")
    } else {
      ag <- d %>%
        group_by(year_num) %>%
        summarise(V = sum(!!msym, na.rm = TRUE), .groups = "drop") %>%
  ...
  output$quarterly_breakdown_chart <- renderPlotly({
    ...
    p <- ggplot(q, aes(Quarter, Revenue, text = paste0(Quarter, "<br>", dollar(Revenue, 1)))) +
  ...
  output$leaderboard_table <- renderDataTable({
    ...
    lb <- d %>%
      group_by(primary_name) %>%
      summarise(Total_Bids   = sum(amt_bid,    na.rm = TRUE),
                Total_Booked = sum(amt_booked, na.rm = TRUE),
                Jobs_Won     = sum(qty_booked, na.rm = TRUE),
                Jobs_Bid     = sum(qty_bid,    na.rm = TRUE),
                .groups = "drop") %>%
      mutate(Win_Rate = Total_Booked / Total_Bids, Avg_Job_Size = Total_Bids / Jobs_Bid)
    ...
    if (input$win_lag > 0) {
      ...
      if (!is.null(lwr)) {
        lag_col <- lwr %>%
          group_by(primary_name) %>%
          summarise(Lag_WR = sum(lag_booked_amt, na.rm = TRUE) / sum(amt_bid, na.rm = TRUE), .groups = "drop")
        lb <- lb %>% left_join(lag_col, by = "primary_name")
      }
    }
    
    lb <- lb %>%
      mutate(Total_Bids   = dollar(Total_Bids,   accuracy = 1),
      ...
      rename(Estimator         = primary_name,
             `Total Revenue`   = Total_Booked,
             `Total Bids`      = Total_Bids,
             `Win % (same-mo)` = Win_Rate,
             `Jobs Won`        = Jobs_Won,
             `Jobs Bid`        = Jobs_Bid,
             `Avg Job Size`    = Avg_Job_Size)
    
    if ("Lag_WR" %in% names(lb))
      lb <- lb %>% mutate(Lag_WR = percent(Lag_WR, accuracy = 0.1)) %>% rename(`Win % (lag-adj)` = Lag_WR)
  ...
  output$preview_table <- renderDataTable({
    ...
    d %>% arrange(desc(report_date)) %>% head(5) %>%
      select(Estimator = primary_name, Date = report_date, Bid = amt_bid, Booked = amt_booked)
  ...
  output$download_report <- downloadHandler(
    ...
    content = function(file) {
      ...
      withProgress(message = "Building Excel report\u2026", value = 0.2, {
        ...
        sel_label <- if (is.null(isolate(input$estimatorSelect)) || length(isolate(input$estimatorSelect)) == 0)
          "All Estimators" else paste(isolate(input$estimatorSelect), collapse = ", ")
        
        meta <- data.frame(
          Field = c("Reporting Period", "Estimator Scope", "Win Rate Lag", "Generated"),
          Value = c(paste(format(isolate(input$dateRange[1]), "%b %d %Y"), "\u2014",
        ...
        kpi_data <- data.frame(
          Metric = c("Total Bid Volume", "Total Booked Revenue", "Dollar Win Rate", "Active Estimators"),
          Value  = c(dollar(sum(d$amt_bid,    na.rm = TRUE), accuracy = 1),
                     dollar(sum(d$amt_booked, na.rm = TRUE), accuracy = 1),
                     percent(sum(d$amt_booked, na.rm = TRUE) / sum(d$amt_bid, na.rm = TRUE), accuracy = 0.1),
        ...
        setColWidths(wb, "Leaderboard", cols = 1:7, widths = c(22, 18, 18, 10, 10, 14, 14))
        lb_raw <- d %>%
          group_by(Estimator = primary_name) %>%
          summarise(`Total Bids`    = sum(amt_bid,    na.rm = TRUE),
        ...
        addStyle(wb, "Leaderboard", dollar_style, rows = 2:(nrow(lb_raw) + 1), cols = c(2, 3, 7), gridExpand = TRUE)
        addStyle(wb, "Leaderboard", pct_style,    rows = 2:(nrow(lb_raw) + 1), cols = 6,           gridExpand = TRUE)
        
        setProgress(0.6)
        
        addWorksheet(wb, "Monthly Detail")
        setColWidths(wb, "Monthly Detail", cols = 1:7, widths = c(22, 14, 14, 10, 14, 10, 14))
        monthly_raw <- d %>%
          arrange(primary_name, report_date) %>%
          mutate(Date     = format(report_date, "%b %Y"),
                 Win_Rate = ifelse(amt_bid > 0, amt_booked / amt_bid, 0)) %>%
        ...
        addStyle(wb, "Monthly Detail", dollar_style, rows = 2:(nrow(monthly_raw) + 1), cols = c(3, 5), gridExpand = TRUE)
        addStyle(wb, "Monthly Detail", pct_style,    rows = 2:(nrow(monthly_raw) + 1), cols = 7,       gridExpand = TRUE)
